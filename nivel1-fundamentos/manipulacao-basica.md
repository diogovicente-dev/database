...
- `Soft Delete` => não efetua a remoção do registro da tabela do banco de dados
  - consite na criação de uma coluna na tabela com o nome `deleted_at` do tipo `timestamp`, por exemplo, que possibilita a manutenção do histórico das informações e faz com que a aplicação possa **ignorar** esse registro.
